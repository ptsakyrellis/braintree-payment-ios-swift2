...
class ViewController: UIViewController, BTDropInViewControllerDelegate {
    ...
    @IBAction func payClick(sender: AnyObject) {
        ...
        let navigationController: UINavigationController = UINavigationController(rootViewController: dropInViewController)
    ...
    func getToken() {
        manager.GET("\(serverBase)/token",
            ...
            success: { (operation: AFHTTPRequestOperation!, responseObject: AnyObject!) -> Void in
                let clientToken = responseObject["clientToken"] as! String
    ...
    func postNonce(paymentMethodNonce: String) {
        let parameters = ["payment_method_nonce": paymentMethodNonce]

        manager.POST("\(serverBase)/payment",
            parameters: parameters,
            success: { (operation: AFHTTPRequestOperation!, responseObject: AnyObject!) -> Void in
                let transactionId : String = (responseObject["transaction"] as AnyObject!)["id"] as! String
